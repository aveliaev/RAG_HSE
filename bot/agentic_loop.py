"""
agentic_loop.py — Настоящий Agentic RAG с итеративным циклом
=============================================================

В отличие от agentic_rag.agentic_ask (одноразовая декомпозиция запроса),
здесь агент работает циклом и сам решает, хватает ли контекста:

  1. Начальный retrieve (через декомпозицию запроса).
  2. LLM-оценка (grade): достаточно ли контекста для ПОЛНОГО ответа.
  3. Если нет — LLM формулирует уточняющий поисковый запрос (refine)
     и делает до-поиск. Накопленные чанки объединяются.
  4. Повтор шагов 2-3 до AGENTIC_MAX_ITERS раз или пока контекст не достаточен.
  5. Финальная генерация ответа по всему накопленному контексту.

Включается флагом USE_AGENTIC_RAG. Если LLM недоступен для оценки —
цикл безопасно вырождается в обычную генерацию по первому retrieve.
"""
